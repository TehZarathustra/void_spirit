...
function Timers:LocateTimer(value)
	for k, timer in pairs(Timers.gameTimeHeap) do
		-- print('lookin at >>>', dump(v))

		if type(value) == "table" then
			if timer == value then return timer end
		elseif type(value) == "string" then
			if timer.name == value then return timer end
		end
...
function Timers:RemoveTimer(name)
	local timerHeap = self.gameTimeHeap
	local timer = Timers:LocateTimer(name)

	if not timer then return end

	-- print('found timer to remove...', dump(timer))
